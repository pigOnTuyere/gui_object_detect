...
class ADP(nn.Module):
    def __init__(self, k=2):
        ...
        self.max = nn.MaxPool2d(kernel_size=k, stride=k)
        self.adv = nn.AvgPool2d(kernel_size=k, stride=k)


    def forward(self, x):
        return self.max(x)+self.adv(x)

# class CAMP(nn.Module):
#     def __init__(self, c1,k=2):
#         super(MP, self).__init__()
#         self.max = nn.MaxPool2d(kernel_size=k, stride=k)
#         self.adv = nn.Advpool2d(kernel_size=k, stride=k)
#         self.conv = nn.Conv2d(c1, c1, k=1, s=1, bias=False)


#     def forward(self, x):
#         lamda = self.
#         self.lamda = torch.clamp(self.lamda , 0, 1)
#         return self.max(x)+self.adv(x)
...
## 
class Yolov7_Tiny_E_ELAN_addDCNv4(nn.Module):
    def __init__(self, inc, ouc, hidc, g, act=True):
        super(Yolov7_Tiny_E_ELAN_addDCNv4, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)
        self.conv2 = Conv(inc, hidc, k=1, act=act)
        self.conv2x = DCNV4_YoLo(hidc, hidc, k=3, g=g, act=act)
        self.conv3 = Conv(hidc, hidc, k=3, g=g, act=act)
        self.conv4 = Conv(hidc, hidc, k=3, g=g, act=act)
        self.conv5 = Conv(hidc * 4, ouc, k=1, act=act)
    ...
    def forward(self, x):
        ...
        x2x= self.conv2x(x2)
        x4 = self.conv4(x3)
        x_concat = torch.concat([x1, x2x, x3, x4], dim=1)
        x_final = self.conv5(x_concat)
        return x_final