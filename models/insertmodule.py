...
class MultiDilatelocalAttention(nn.Module):
    ...
    def __init__(self, c1,c2,dim=80, num_heads=8, qkv_bias=False, qk_scale=None,
    ...
    def forward(self, x):
        ...
        x_new = []
        for i in range(self.num_dilation):
            x_new.append(self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2]))  # B, H, W,C//num_dilation
...
if __name__ == "__main__":
    x = torch.rand([2, 256, 80, 32]).cuda()
    m = MultiDilatelocalAttention(32,32,32).cuda()
    y = m(x)
    print(y.shape)
